...
$id = $_GET["id"];
$read_sql = "SELECT * FROM jaket WHERE id = $id";
...
if (isset($_POST["submit"])) {
    ...
    $update_sql = "UPDATE jaket SET barang = '$barang', stok='$stok', harga='$harga', restock = '$restock' WHERE id='$id'";
    ...
    if ($result) {
        ...
        echo "<script>
            ...
            document.location.href = 'update3.php';
        </script>";
    }
}
?>
...
<div class="kotak_login">
    <h1 class="title">Update Data Jaket</h1>
    <form action="" method="post">
        <input type="hidden" name="id" value="<?= $jaket["id"]; ?>">
        <label for="barang">Barang</label><br>
        <input type="text" class="form_update" name="barang" id="barang" value="<?= $jaket["Barang"]; ?>"><br><br>
        <label for="stok">Stok</label><br>
        <input type="text" class="form_update" name="stok" id="stok" value="<?= $jaket["Stok"]; ?>"><br><br>
        <label for="harga">Harga</label><br>
        <input type="number" class="form_update" name="harga" id="harga" value="<?= $jaket["Harga"]; ?>"><br><br>
        <label for="restock">Restock date(YYYY-MM-DD): </label>
        <input type="text" class="form_update" name="restock" id="restock" value="<?= $jaket["Restock"]; ?>"><br><br>
